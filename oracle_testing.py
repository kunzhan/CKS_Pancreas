...
result_path = os.path.join(result_path, 'oracle_testing_' + \
	sys.argv[11] + 'x' + str(learning_rate_m1) + ',' + str(crop_margin))
...
snapshot_name = snapshot_name_from_timestamp(snapshot_path, \
	current_fold, plane, 'S', slice_thickness, organ_ID, timestamp)
if snapshot_name == '':
...
for t in range(len(snapshot)):
	...
	if not finished:
		net = RSTN(crop_margin=crop_margin, crop_prob=crop_prob, \
					crop_sample_batch=crop_sample_batch, TEST='O').cuda()
		...
		if not os.path.isfile(volume_file):
			...
			for j in range(minR, maxR):
				...
				if plane == 'X':
					...
					label_ = label[sID, :, :].astype(np.float32)
				elif plane == 'Y':
					if label_sumY[sID].sum() == 0:
						continue
					image_ = image[:, sID, :].transpose(1, 0, 2).astype(np.float32)
					label_ = label[:, sID, :].transpose(1, 0, 2).astype(np.float32)
				elif plane == 'Z':
					if label_sumZ[sID].sum() == 0:
						continue
					image_ = image[:, :, sID].transpose(2, 0, 1).astype(np.float32)
					label_ = label[:, :, sID].transpose(2, 0, 1).astype(np.float32)

				image_ = image_.reshape((1, 3, image_.shape[1], image_.shape[2]))
				image_ = torch.from_numpy(image_).cuda().float()
				label_ = label_.reshape((1, 3, label_.shape[1], label_.shape[2]))
				label_ = torch.from_numpy(label_).cuda().float()
				out = net(image_,1, label_).data.cpu().numpy()[0, :, :, :]
